...
function initDb() {
    return new sqlite3.Database(path.join(__dirname, common.config.database_file_path));
}
...
exports.SpAddNewMovie = function(data, img_url, callback) {
    ...
    db.get(sql1, [], (err, row) => {
        ...
        db.all(sql2, ["" + sno, img_url, data.ddlStatus, data.txtTitle, data.txtReleaseDate, data.txtDirector, data.txtCasts, data.ddlLanguage, data.ddlIndustry], (err, rows) => {
            if (err) {
                throw err;
            }
            callback(rows);
